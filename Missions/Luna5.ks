...
If runmode = 2.2{
	...
	local Cirdv is ff_CircOrbitVel(ship:orbit:periapsis) - ff_EccOrbitVel(ship:orbit:periapsis, ship:orbit:semimajoraxis).
...
If runmode = 3{
	...
	Until counter > 120{
		Clearscreen.
		Print "Refine Node to start descent burn before: " + (120-counter).
		wait 1.
		Set Counter to counter +1.
	}
	set runmode to 3.1.
...
If runmode = 3.1{
	...
	Set corr_time to (time:seconds + nextnode:eta).
	Print nextnode:eta.
	Print corr_time. 
	wait 5.
...
If runmode = 4{
	...
	wait 2.
	Print "Suburn 750".
	ff_SuBurn(281.7, 3.6, 0.1, 500).
	wait 5.
	Print "Suburn 100".
	ff_SuBurn(281.7, 3.6, 0.1, 50).
	wait 2.
	Print "Suburn 5".
	ff_SuBurn(281.7, 3.6).
	SET SHIP:CONTROL:FORE to 0.
...
If runmode = 5{
	...
	until ((ALT:RADAR > 10000) or (altitude > 15000)) and (SHIP:GROUNDSPEED > 300) {
		wait 1.
	}
	Print "Move to 10: "+ altitude.
	LOCK STEERING to HEADING(90,10).
	until (Ship:apoapsis > 30000) and (SHIP:GROUNDSPEED > 1200){
		wait 1.
	}
...
If runmode = 6{
	...
	Add n.
	local startTime is time:seconds + nextnode:eta - (ff_Burn_Time(nextnode:deltaV:mag / 2, 281.7, 3.6, 1)).
	lock steering to nextnode:burnvector.
	wait until time:seconds > startTime.
	lock throttle to 1.
	wait 10.
...
If runmode = 8{
	...
	Set warp to 0.
	wait 45.
	until ship:periapsis < 70000{ //75 km for lunar rated 97.5km for non lunar rated.
...
If runmode = 9{
	print "awaiting atmosphere".
	ff_Avionics_off().
	until altitude < 200000 {
		wait 5.
	}
	print "atmosphere loop".
	until ship:apoapsis < 150000 {
		Lock steering to retrograde.
		if ship:periapsis < 97500{
			ff_Avionics_on().
			rcs on.
			wait 30.
		...
		until ship:altitude > 140000{
			...
			If ADDONS:Available("KAC") {		  // if KAC installed	  
				Local ALM is ADDALARM ("Maneuver", (time:seconds + eta:periapsis - 600), SHIP:NAME ,"").// creates a KAC alarm 3 mins prior to the manevour node
			}
		}
	}
	Set runmode to 10.
...
Function ff_CAB{ 
	...
	Until (SHIP:GROUNDSPEED < 800) or (ALT:RADAR < 5000){
		//Create PID to adjust the craft pitch (without thrusting downward) which maintains a vertical velocity of zero and regulates the velocity of burn height change if not zero reventing a pitch above the horizontal.		
	...
	Until (SHIP:GROUNDSPEED < 50) or (ALT:RADAR < 2500){
		//Create PID to adjust the craft pitch (without thrusting downward) which maintains a vertical velocity of zero and regulates the velocity of burn height change if not zero reventing a pitch above the horizontal.		
		Set dpitch TO PIDVV:UPDATE(TIME:SECONDS, verticalspeed). //Get the PID on the AlT diff as desired vertical velocity
	...
	Until (SHIP:GROUNDSPEED < 10) or (ALT:RADAR < 2000){
		wait 0.01.
	}
...
Function ff_SuBurn {
	...
	set Flight_Arr to hf_fall(EngISP, EngThrust).
	Lock steering to (-1) * SHIP:VELOCITY:SURFACE.
	wait 0.05.
	Until (Flight_Arr["fallDist"] + SafeAlt + (ThrottelStartUp * abs(ship:verticalspeed))) > (ship:Altitude - SHIP:GEOPOSITION:TERRAINHEIGHT) { // until the radar height is at the suicide burn height plus safe altitude and an allowance for the engine to throttle up to max thrust
		...
		Print "gl_fallBurnTime:" + ff_burn_time(Flight_Arr["fallVel"],EngISP, EngThrust, 1).
