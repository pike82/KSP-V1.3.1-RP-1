...
parameter T2 is 99.8.
	...
	local s_pitch is 0.
	local w_T1 is w*1.2. //first guess at tgt_w which is actually the current w pluis 20%.
...
function f_PEG_ABT_2 {
	...
	//Until false{  
		...
		//help convergence bounds
		// if rT1 > tgt_r{
		// 	Print "A1 decrease: " + abs(A1*0.05).
		// 	Print A1.
		// 	Print rT1.
		// 	Print tgt_r.
		//  	Set A1 to A1 - abs(A1*0.05). //if estimate higher than target height lower A1.
		// }
		// if rT1 < s_r{
		// 	Print "A1 increase: " + abs(A1*0.05).
		// 	Print A1.
		// 	Print rT1.
		// 	Print tgt_r.
		//  	Set A1 to A1 + abs(A1*0.05). //if estimate lower than current height raise A1.
		// }

	/// determine vertical state for first stage
		local bb01 is -s_ve*(LN(1-(T1/tau))).  //L1
		local bb11 is (bb01 * tau) - (s_ve*T1).  //J1
		local cc01 is (bb01*T1)-bb11. //S1
		local cc11 is (cc01*tau) - ((s_ve*(T1^2))/2).//Q1
