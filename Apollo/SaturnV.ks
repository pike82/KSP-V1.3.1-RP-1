...
If runmode = 0{
	...
	//Wait for launch window //16/07/1969 at 13:32:00 GMT 19 202 13 32 00  which is apollo 11
	Set vYear to 19.
	Set vDay to 202.
	Set vHour to 13.
	Set vMinute to 31.
	...
	Local RO_Engine_offset is 2.32. //2.32 to allow for RO engine startup delay compared to real engines
...
// https://amyparent.com/post/automating-rocket-launches/
    ...
    until false {
		...
		if (time:seconds > (liftoff + 719.3)) and (I = 8){
			...
			Wait until ship:orbit:eccentricity < 0.00021 or time:seconds > (liftoff + 796.5).
			FOR eng IN engList { 
				IF eng:TAG ="APS" { 
					eng:shutdown.
				}
...
function f_TLI{
	// ending at 10203.03
	// earth fixed 10,433.3 m/s
	// 320.9 km altitude
	// 10,841.2 m/s spaced fixed
	//0.97548 eccentricity
	//31.386 inclination
	//121.849 DN

	// ends at 10,213.03 sec
	//334.4km
	//10,834.3 km
	//60.073 heading
	//31.383 inclination
	...
	local mnv is node ( (basetime + 9856 + (10203 - 9856)  + 2700 ), -10, -179, 3192 ). //r,n,p
	add mnv.
	
	// 10,213.03 TLI
	//11,723 CSM seperation

	//note as KSP has the moon 180 Degrees opposite to in real life. all figures will be 45 minutes later than real.

	local I is 1.
	until false{
		//9278.2 restart preps (2:34:38) 202/15/32/57
		if (time:seconds > (liftoff + 9278.2 + 2700)) and (I = 1){
			RCS on.
			SAS on.
			unlock steering.
			wait 1.
		...
		//9775 ullage (2:42:55) 
		if (time:seconds > (liftoff + 9775 + 2700)) and (I = 2){
			FOR eng IN engList { 
				IF eng:TAG ="APS" { 
					eng:activate.
				}
		...
		//9850 ullage stop (2:44:10)
		if (time:seconds > (liftoff + 9850 + 2700)) and (I = 3){
			FOR eng IN engList { 
				IF eng:TAG ="APS" { 
					eng:shutdown.
				}
		...
		//9856.2 engine reignition (2:44:15)
		if (time:seconds > (liftoff + 9856.2 + 2700)) and (I = 4){
			FOR eng IN engList { 
				IF eng:TAG ="J2-1" { 
					eng:activate.
				}
		...
		if (time:seconds > (liftoff + 9972.2 + 2700)) and (I = 5){
			FOR eng IN engList { 
				IF eng:TAG ="J2-1" { 
					Local M is eng:GETMODULE("EMRController").
					Print M:GETFIELD("current EMR").
		...
		// 10,203.03 second engine cut-off
		if (time:seconds > (liftoff + 10203.03 + 2700)) and (I = 6){
			FOR eng IN engList { 
				IF eng:TAG ="J2-1" { 
					eng:shutdown.
				}
...
function f_f1_thrust {
	return max(0.001,((ship:AVAILABLETHRUST/(ship:mass*9.81))-1.20)/1.4).//2.6 to 1.20 TWR range // was 3.0
}
